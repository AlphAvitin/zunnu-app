...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const pet = await get('SELECT * FROM lost_pets WHERE id = ?', [result.lastInsertRowid]);
    res.json(pet);
  } catch (err) {
    res.status(500).json({ error: 'Erro interno' });
  }
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    const pet = await get('SELECT * FROM lost_pets WHERE id = ?', [req.params.id]);
    ...
    res.json(await get('SELECT * FROM lost_pets WHERE id = ?', [req.params.id]));
  } catch (err) {
    res.status(500).json({ error: 'Erro interno' });
  }
});
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    await run('DELETE FROM lost_pets WHERE id = ?', [req.params.id]);
