...
router.post('/', (req, res) => {
  try {
    ...
    const product = get('SELECT * FROM products WHERE id = ? AND is_active = 1', [productId]);
    if (!product) return res.status(404).json({ error: 'Produto nao encontrado' });

    const existing = get('SELECT * FROM cart_items WHERE user_id = ? AND product_id = ?', [req.userId, productId]);
    if (existing) {
      run('UPDATE cart_items SET quantity = quantity + ? WHERE id = ?', [quantity || 1, existing.id]);
    } else {
      run('INSERT INTO cart_items (user_id, product_id, quantity) VALUES (?, ?, ?)', [req.userId, productId, quantity || 1]);
    }
...
router.put('/:id', (req, res) => {
  try {
    const item = get('SELECT * FROM cart_items WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
