...
router.get('/profile', authMiddleware, async (req, res) => {
  try {
    const profile = await get('SELECT * FROM business_profiles WHERE user_id = ?', [req.userId]);
...
router.post('/profile', authMiddleware, async (req, res) => {
  try {
    ...
    const existing = await get('SELECT * FROM business_profiles WHERE user_id = ?', [req.userId]);
    if (existing) {
      await run(
    ...
    res.json(await get('SELECT * FROM business_profiles WHERE user_id = ?', [req.userId]));
  } catch (err) {
    res.status(500).json({ error: 'Erro interno' });
  }
});
