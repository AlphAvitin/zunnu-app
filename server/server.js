...
async function start() {
  ...
  if (userCount && userCount.c === 0) {
    ...
    for (const u of users) run('INSERT OR IGNORE INTO users (name, email, password_hash, bio, location, plan, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', u);
    const pets = [
    ...
    for (const p of pets) run('INSERT OR IGNORE INTO pets (user_id, name, species, breed, age, location, is_castrated, zodiac, image, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', p);
    const vaccines = [
    ...
    for (const v of vaccines) run('INSERT OR IGNORE INTO vaccines (pet_id, name, date, status) VALUES (?, ?, ?, ?)', v);
    ...
    for (const r of reels) run('INSERT OR IGNORE INTO reels (user_id, video_url, caption, music) VALUES (?, ?, ?, ?)', r);
